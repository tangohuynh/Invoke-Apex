...
.PARAMETER SysInfo
	...
	Compiles a .NET Aseembly that Dumps everything from the "Invoke-Sysinfo" function.
...
"@
	...
	elseif ($WiFiCreds) {
		...
		$CompilerArgs = "/r:$SmaDll /t:exe /out:C:\windows\temp\WiFiCreds.exe $CsFile"
...
"@
		...
		Sleep 4
		Remove-Item $env:temp\$Z.cs
		Write "`n [+] Assembly --> C:\windows\temp\WiFiCreds.exe`n"
	}
	elseif ($SysInfo) {
